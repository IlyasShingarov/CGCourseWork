...
public class GouraudShader {
    ...
    public Vector3d lightDir = new Vector3d(0, 0, 0.2);
...
//        System.out.println("v_n " + nthvert + "\nn " + n + "\n");
        switch (nthvert){
            case 0:
                varyingIntensity.x = Math.max(0., newNorm.dot(lightDir));
                break;
            case 1:
                varyingIntensity.y = Math.max(0., newNorm.dot(lightDir));
                break;
            case 2:
                varyingIntensity.z = Math.max(0., newNorm.dot(lightDir));
                break;
        }
...
//        intensitySmooth = Math.min(255, Math.max(0,255*intensitySmooth));
        intensitySmooth = Math.min(255, Math.max(0,50*intensitySmooth));
